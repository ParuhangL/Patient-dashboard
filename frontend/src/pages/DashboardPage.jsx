  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 28 }}>
        <StatCard icon={Activity}     label="Avg Systolic BP"  value={summary.avg_bp_systolic ? `${summary.avg_bp_systolic}` : '—'} sub="mmHg"  color="#ef4444" />
        <StatCard icon={Droplets}     label="Avg Glucose"      value={summary.avg_glucose     ? `${summary.avg_glucose}`     : '—'} sub="mg/dL" color="#8b5cf6" />
        <StatCard icon={Heart}        label="Avg BMI"          value={summary.avg_bmi         ? `${summary.avg_bmi}`         : '—'} sub="kg/m²" color="#ec4899" />
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          {pieData.every(d => d.value === 0) ? (
            <div style={{ color: '#64748b', fontSize: 13, textAlign: 'center', padding: '40px 0' }}>
              No risk data yet — upload a dataset to see results
            </div>
          ) : (
